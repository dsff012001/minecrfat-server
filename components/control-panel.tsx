...
export function ControlPanel() {
  ...
  return <div className="min-h-svh bg-background font-sans text-foreground">
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-20 flex h-16 items-center border-b bg-background/95 px-4 backdrop-blur md:px-6"><Button variant="ghost" size="icon" className="lg:hidden" onClick={()=>setMobileOpen(true)} aria-label="Menüyü aç"><Menu/></Button><div className="ml-auto flex items-center gap-2"><SupportCenter/><div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="size-2 rounded-full bg-primary"/>Panel çevrimiçi</div></div></header><main className="p-4 md:p-6 lg:p-8"><InfrastructureManager/></main></div>
  </div>
}
