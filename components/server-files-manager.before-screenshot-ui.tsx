...
import { Archive, Check, FileCog, FileUp, Loader2, Search, Trash2, UploadCloud } from 'lucide-react'
...
export function ServerFilesManager({ serverId, disabled }: { serverId: string; disabled?: boolean }) {
  ...
  return <Card className="border-slate-700 bg-[#14231c] text-slate-100"><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>Dosyalar ve yüklemeler</CardTitle><CardDescription className="text-slate-400">Mod, plugin, config ve dünya dosyalarını tek yerden yönetin.</CardDescription></div><Button variant="outline" onClick={() => inputRef.current?.click()} disabled={disabled || busy}><FileUp className="mr-2 size-4" />Dosya seç</Button></div></CardHeader><CardContent className="space-y-5"><input ref={inputRef} type="file" className="hidden" accept=".jar,.zip,.yml,.yaml,.json,.properties,.toml,.ini,.cfg,.conf,.txt" onChange={event => { const selected = event.target.files?.[0]; if (selected) void inspect(selected) }} /><button type="button" disabled={disabled || busy} onDragOver={event => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); const dropped = event.dataTransfer.files[0]; if (dropped) void inspect(dropped) }} onClick={() => inputRef.current?.click()} className={`flex min-h-40 w-full flex-col items-center justify-center gap-3 rounded-xl border border-dashed p-6 text-center transition ${dragging ? 'border-emerald-300 bg-emerald-400/10' : 'border-slate-600 bg-[#0d1713]'}`}><UploadCloud className="size-8 text-emerald-300" /><span className="font-medium">Tek dosya veya ZIP sürükleyin</span><span className="text-sm text-slate-400">ZIP içeriği klasör ve uzantıya göre mods, plugins, config veya worlds klasörlerine dağıtılır.</span></button>{file && <div className="rounded-lg border border-slate-700 bg-[#0d1713] p-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><Archive className="size-5 text-emerald-300" /><div><p className="font-medium">{file.name}</p><p className="text-xs text-slate-400">{(file.size / 1048576).toFixed(2)} MB</p></div></div><Button onClick={() => void upload(Boolean(preview))} disabled={busy}>{busy ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Check className="mr-2 size-4" />}{preview ? 'Önizlemeyi onayla' : 'Yükle'}</Button></div>{preview && <div className="mt-4 grid gap-2 text-sm"><p className="font-medium">{preview.entries.length} dosya bulundu</p>{preview.entries.slice(0, 12).map(entry => <div key={entry.name} className="flex justify-between rounded bg-slate-800/60 px-3 py-2"><span className="truncate">{entry.name}</span><span className="text-emerald-300">{categoryLabel[entry.category]}</span></div>)}{preview.rejected.length > 0 && <p className="text-amber-300">{preview.rejected.length} dosya reddedilecek.</p>}</div>}</div>}{notice && <p role="status" className="rounded-md border border-emerald-400/30 bg-emerald-950/20 p-3 text-sm text-emerald-200">{notice}</p>}<div className="flex items-center gap-2"><Search className="size-4 text-slate-400" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Dosya ara" className="border-slate-600 bg-[#202a32]" /></div>{error ? <p className="text-red-300">{error.message}</p> : records.length === 0 ? <p className="rounded-lg border border-dashed border-slate-700 p-6 text-center text-sm text-slate-400">Henüz yükleme kaydı yok.</p> : <div className="grid gap-3">{records.map(record => <div key={record.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-700 bg-[#0d1713] p-4"><div className="min-w-0"><p className="truncate font-medium">{record.payload.filename ?? record.type}</p><p className="text-xs text-slate-400">{record.type} · {record.status} · {new Date(record.createdAt).toLocaleString('tr-TR')}</p>{record.result?.error && <p className="text-xs text-red-300">{record.result.error}</p>}</div><Button size="icon" variant="ghost" aria-label="Dosyayı sil" disabled={disabled || busy || !recordPath(record)} onClick={() => void removeRecord(record)}><Trash2 className="size-4 text-red-300" /></Button></div>)}</div>}<p className="flex items-center gap-2 text-xs text-slate-500"><FileCog className="size-4" />Yüklenen dosyalar agent kuyruğunda görünür; metin düzenleme ve silme işlemleri yalnızca sunucu durdurulduğunda çalışır.</p></CardContent></Card>
}
