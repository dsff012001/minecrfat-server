...
if [[ "$(. /etc/os-release && echo "$ID:$VERSION_ID")" != "ubuntu:24.04" && "$(. /etc/os-release && echo "$ID:$VERSION_ID")" != "ubuntu:22.04" && "$(. /etc/os-release && echo "$ID:$VERSION_ID")" != "ubuntu:20.04" ]]; then echo "This installer targets Ubuntu 20.04, 22.04 or 24.04." >&2; exit 1; fi
...
apt-get update
apt-get install -y ca-certificates curl unzip tar git nginx ufw openssh-server acl openjdk-21-jre-headless
...
Restart=always
RestartSec=5
# SFTP/DB privileged helpers are narrowly allowlisted in sudoers.
...
if [[ -f "$(dirname "$0")/install-sftp-helper.sh" ]]; then bash "$(dirname "$0")/install-sftp-helper.sh"; else echo 'install-sftp-helper.sh not found; install it separately before provisioning SFTP.' >&2; fi
ufw allow OpenSSH
ufw allow 25565/tcp
ufw --force enable
